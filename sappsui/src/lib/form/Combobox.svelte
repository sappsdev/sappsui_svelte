<script lang="ts" generics="T">
	...
	import type { SearchState } from '$lib/hooks/use-search.svelte.js';
		...
		{#if (srcKey && selected?.[srcKey]) || (iconKey && selected?.[iconKey])}
			<div class="field-start">
				<Avatar
					...
					icon={selected?.[iconKey] as IconName}
					size={avatarSizes[size]}
				/>
			</div>
		{/if}

		<div class="field-center" class:invisible={!selected && !isOpen}>
			{selected?.[labelKey] ?? placeholder}
		</div>

		<div class="field-btn">
			<Icon name={arrowIcon} class={cn('field-arrow', isOpen && 'is-active')} />
		</div>
	</button>
