<script lang="ts">
	...
	import { Footer, Checkbox, IconButton, FooterLinks } from 'sappsui';
	...
	let code = $derived(() => {
		const scriptLines = [
			`<script lang="ts">`,
			`\timport { Footer, IconButton, FooterLinks } from 'sappsui';`,
			showCenter && `\n\tconst footerLinks = [`,
		...
		const componentLines = [
			...
			hasProps && `>`,
			!hasProps && `<Footer class="relative">`,
			showStart && `\t{#snippet start()}`,
			showStart && `\t\t<p class="text-sm text-muted-foreground">© 2024 Company</p>`,
			showStart && `\t{/snippet}\n`,
