<script lang="ts">
	...
	import { AppBar, Checkbox, IconButton, Dropdown, NavMenu } from 'sappsui';
	...
	// NavMenu items for center content
	const navItems = [
	...
	// Dropdown options for profile menu
	...
	let code = $derived(() => {
		const scriptLines = [
			`<script lang="ts">`,
			`\timport { AppBar, IconButton, Dropdown, NavMenu } from 'sappsui';`,
