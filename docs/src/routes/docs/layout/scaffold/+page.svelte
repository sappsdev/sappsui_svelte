<script lang="ts">
	...
	import { Scaffold, AppBar, Sidebar, Card, Checkbox, Select } from 'sappsui';
	...
	let code = $derived(() => {
		const scriptLines = [
			`<script lang="ts">`,
			`\timport { Scaffold, AppBar, Sidebar, Card } from 'sappsui';`,
			`<\/script>`
		].filter(Boolean);
...
{#snippet builder()}
	<Select label="vh Utility" name="vh" size="sm" options={vhOptions} bind:value={vhUtility} />

	<DocOptions title="Sections">
		<Checkbox bind:checked={showAppBar} name="show-appbar" label="AppBar" />
		<Checkbox bind:checked={showBottomBar} name="show-bottombar" label="BottomBar" />
		<Checkbox bind:checked={showStart} name="show-start" label="Start Sidebar" />
		<Checkbox bind:checked={showEnd} name="show-end" label="End Sidebar" />
	</DocOptions>
{/snippet}
