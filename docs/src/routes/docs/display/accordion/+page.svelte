<script lang="ts">
	...
	import { Accordion, Select, Checkbox } from 'sappsui';
	...
	let accordionItems: any = $derived.by(() => {
		return [
			{
				id: '1',
				label: 'What is SappsUI?',
				icon: withIcon ? 'fluent:info-24-regular' : undefined,
				content: withSnippet
					? content1
					: 'SappsUI is a modern component library built with Svelte 5, offering a comprehensive set of customizable UI components for building beautiful web applications.',
			...
			},
			{
				...
				content: withSnippet
					? content2
					: 'You can install SappsUI using npm or pnpm. Simply run "npm install sappsui" in your project directory and start using the components right away.',
			...
			},
			{
				...
				content: withSnippet
					? content3
					: 'Yes! SappsUI is completely free and open source. You can use it in both personal and commercial projects without any restrictions.'
			}
		];
	});
	...
	let code = $derived(() => {
		const scriptLines = [
			`<script lang="ts">`,
			`\timport { Accordion } from 'sappsui';`,
			withSnippet && `\n\tconst items = [`,
			withSnippet && `\t\t{ id: '1', label: 'What is SappsUI?', content: content1 },`,
			...
			!withSnippet && `\t\t{`,
			!withSnippet && `\t\t\tid: '1',`,
			!withSnippet && `\t\t\tlabel: 'What is SappsUI?',`,
			!withSnippet && withIcon ? `\t\t\ticon: 'fluent:info-24-regular',` : '',
			!withSnippet &&
				`\t\t\tcontent: 'SappsUI is a modern component library...'${withDefaultOpen ? ',\n\t\t\tdefaultOpen: true' : ''}`,
			!withSnippet && `\t\t},`,
			!withSnippet && `\t\t{`,
			!withSnippet && `\t\t\tid: '2',`,
			...
			!withSnippet &&
				`\t\t\tcontent: 'You can install SappsUI using npm...'${withDisabled ? ',\n\t\t\tdisabled: true' : ''}`,
			!withSnippet && `\t\t},`,
			!withSnippet && `\t\t{`,
			!withSnippet && `\t\t\tid: '3',`,
			!withSnippet && `\t\t\tlabel: 'Is it free to use?',`,
			!withSnippet && withIcon ? `\t\t\ticon: 'fluent:money-24-regular',` : '',
			!withSnippet && `\t\t\tcontent: 'Yes! SappsUI is completely free...'`,
			!withSnippet && `\t\t}`,
			!withSnippet && `\t];`,
			`<\/script>`,
			withSnippet && `\n{#snippet content1()}`,
			withSnippet && `\t<div>`,
			withSnippet && `\t\t<p><strong>SappsUI</strong> is a modern component library.</p>`,
			withSnippet && `\t\t<ul>`,
			...
			withSnippet && `{/snippet}`,
			withSnippet && `\n{#snippet content2()}`,
			withSnippet && `\t<p>Install using: <code>npm install sappsui</code></p>`,
			withSnippet && `{/snippet}`,
			withSnippet && `\n{#snippet content3()}`,
...
{#snippet content1()}
	<div>
		<p><strong>SappsUI</strong> is a modern component library built with Svelte 5.</p>
...
{#snippet content2()}
	<div>
		<p>You can install SappsUI using npm or pnpm:</p>
		<code class="block mt-2 p-2 bg-muted rounded">npm install sappsui</code>
...
{#snippet content3()}
	<div>
		<p>
			Yes! SappsUI is <em>completely free</em> and <strong>open source</strong>. You can use it in
