<script lang="ts">
	...
	import { Collapsible, Select, Checkbox } from 'sappsui';
	...
	let code = $derived(() => {
		const scriptLines = [
			`<script lang="ts">`,
			`\timport { Collapsible } from 'sappsui';`,
			!withSnippet && `<\/script>`,
			withSnippet && `<\/script>`,
			withSnippet && `\n{#snippet content()}`,
			withSnippet && `\t<div>`,
		...
		const componentLines = [
			hasProps && `\n<Collapsible`,
			hasProps && `\tlabel="What is SappsUI?"`,
			withIcon && `\ticon="fluent:info-24-regular"`,
			withSnippet && `\tcontent={content}`,
			!withSnippet &&
				`\tcontent="SappsUI is a modern component library built with Svelte 5, offering a comprehensive set of customizable UI components."`,
