<script lang="ts">
	...
	const searchTransform = useSearch({
		url: '/api/search',
		debounceSearch: 500,
		minSearchLength: 2,
		transformData: (data) => ({
...
<Section>
	<div class="prose">
		<h1>Combobox</h1>
		<p>Combobox component is used to get input from a list of options with search capability.</p>
	</div>
  ...
  const searchState = useSearch({
    url: '/api/users',
    buildQueryParams: (search) => ({
      q: search,           // Personalizar el nombre del parámetro
      fields: 'name,email', // Campos a buscar
      limit: '20',         // Límite de resultados
      status: 'active'     // Filtros adicionales
    }),
    debounceSearch: 300
  });
<\/script>
