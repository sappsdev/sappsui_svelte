<script lang="ts">
	import { IconButton, Card, Checkbox, Code, Section, Divider, Select } from 'sappsui';
	const themeColors = [
...
<Section contentClass="gap-4 p-3">
	<div class="prose">
		<h1>IconButton</h1>
		<p>IconButtons allow users to take actions with a single tap.</p>
	</div>
	<Card contentClass="column gap-4">
		...
		{#snippet footer()}
			<Code
				lang="svelte"
				code={`<script lang="ts">
  import { IconButton } from 'sappsui'; ${!isLink ? '\n  const handleClick = () => console.log("clicked");' : ''}
<\/script>
<IconButton
  icon="solar:cloud-rain-outline"
