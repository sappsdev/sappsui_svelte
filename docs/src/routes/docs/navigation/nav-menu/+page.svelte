<script lang="ts">
	...
	import { NavMenu, Select, Checkbox, Section } from 'sappsui';
	...
	let code = $derived(() => {
		const scriptLines = [`<script lang="ts">`, `\timport { NavMenu } from 'sappsui';`];

		if (withMegamenu) {
			scriptLines.push(
				`\n\t// Define megamenu snippet`,
				`\tconst megamenuSnippet = () => {`,
				`\t\treturn \`<div class="grid grid-cols-3 gap-6 p-4 min-w-[600px]">`,
